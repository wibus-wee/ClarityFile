import type { DroppedFileInfo, DocumentImportConfig, ImportHandlerResult } from './types'
 ...
 */
export class DocumentImportHandler {
  /**
   * 处理文档导入
   * @param files 拖拽的文件列表
   * @param config 导入配置
  ...
  static async handleImport(
    ...
    try {
      ...
      if (config.forceCreateNew) {
        // 强制创建新文档
        await this.createNewDocumentWithVersion(file, config)
      } else if (config.forceAddVersion) {
        // 强制为现有文档添加版本，需要用户选择文档
        await this.selectExistingDocumentForVersion(file, config)
      } else {
        // 自动检查（保留原有逻辑，但现在不会被使用）
        const existingDocument = await this.findExistingDocument(file, config.projectId)

        if (existingDocument) {
          // 如果有现有文档，直接添加版本
          await this.addVersionToExistingDocument(file, existingDocument, config)
        } else {
          // 如果没有现有文档，创建新文档
          await this.createNewDocumentWithVersion(file, config)
        }
  ...
  private static async findExistingDocument(
    ...
    try {
      ...
      // 根据文件名推断文档名称
      const inferredDocumentName = this.inferDocumentNameFromFile(file)
   ...
   * 选择现有文档并添加版本
   */
  private static async selectExistingDocumentForVersion(
    file: DroppedFileInfo,
    config: DocumentImportConfig
  ): Promise<void> {
    try {
      ...
      await this.addVersionToExistingDocument(file, selectedDocument, config)
    } catch (error) {
   ...
   * 为现有文档添加版本
   */
  private static async addVersionToExistingDocument(
    file: DroppedFileInfo,
    document: any,
    config: DocumentImportConfig
  ): Promise<void> {
    ...
    // 生成版本标签
    const versionTag = config.versionTag || this.generateVersionTag(documentWithVersions.versions)

    // 使用全局状态管理打开 DocumentVersionFormDrawer
    const { openDocumentVersionForm } = useGlobalDrawersStore.getState()
   ...
   * 创建新文档并添加第一个版本
   */
  private static async createNewDocumentWithVersion(
    file: DroppedFileInfo,
    config: DocumentImportConfig
  ): Promise<void> {
    // 推断文档信息
    const inferredDocumentName = config.logicalDocumentName || this.inferDocumentNameFromFile(file)
    const inferredDocumentType = config.logicalDocumentType || this.inferDocumentTypeFromFile(file)

    // 使用全局状态管理打开 DocumentDrawer
    const { openDocumentForm } = useGlobalDrawersStore.getState()
    ...
    // 暂存文件信息，用于后续创建版本
    // 这需要在全局状态中添加一个临时存储
    const { updateDocumentForm } = useGlobalDrawersStore.getState()
    updateDocumentForm({
      preselectedFile: file
    })
   ...
   * 从文件名推断文档名称
   */
  static inferDocumentNameFromFile(file: DroppedFileInfo): string {
   ...
   * 从文件名推断文档类型
   */
  static inferDocumentTypeFromFile(file: DroppedFileInfo): string {
   ...
   * 生成版本标签
   */
  static generateVersionTag(existingVersions: any[]): string {
   ...
   * 检查文件是否为文档文件
   */
  static isDocumentFile(file: DroppedFileInfo): boolean {
